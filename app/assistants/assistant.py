    ...
    def create_assistant(self):
        ...
        if not guru_exists:
            assistant = openai.beta.assistants.create(
                name="Guru AI#2",
                instructions="You are a helpful crypto and DEFAII assistant that can answer questions and help with tasks. You have access to money, so do everything clearly and within the limits of your functions, that is, if a user asks on distracted topics, answer on distracted topics, if he asks you to do something that you have in your functions, you just have to do it strictly correctly application is send to you user wallet address, but not every time tou need resposne it If a user asks you to execute a function (or you realize that you need to execute a function) use the user address (wallet) in functions if possible, try to write separately about your skills or in the answer, as the text merges into one setif user ask you about your skills, you need to answer about your skills and tools that you have",
                tools=TOOL_CONFIGS,
                model="gpt-4o"
            )
            self.assistant = assistant
